...
#include <netinet/in.h> /* for inet_ntoa_r() */
...
/*
 *  Contains information about connected clients. Passed to each thread
 */
...
/*
 * Executed by threads. Serves client and joins to main on completion
 */
void * handleClient(void *data_t){

        //Contains information of client for each thread  
        tdata *threadData = (tdata *)data_t;
        ...
        strcpy(address,inet_ntoa((threadData->clntAddr).sin_addr));  

        //Allow file functions on client sock
        FILE *clntFp = fdopen(threadData->clntSock, "r");
        if (clntFp == NULL)
            die("fdopen failed");
        ...
        //Note implementation of reenterant strtok_r() rather than strtok()
        ...
        if (!method || !requestURI || !httpVersion || 
            ...
            sendStatusLine(threadData->clntSock, statusCode);
            goto loop_end;
        }
        ...
        statusCode = handleFileRequest(threadData->webRoot, requestURI, threadData->clntSock);
...
loop_end:
        ...
        //Detach thread and free tdata socket and thread ID
        pthread_detach(pthread_self());
        free(threadData->threadid);
        free(threadData);
...
int main(int argc, char *argv[])
{
    ...
    //creating server socket and getting webroot
    ...
    for (;;) {
        ...
        // Set struct tdata to pass client information to thread
        tdata *threadData = (tdata *)malloc(sizeof(tdata));
        tdata dataCopy = {(pthread_t *)0,clntSock, webRoot, clntAddr}; 
        dataCopy.threadid = (pthread_t *)malloc(sizeof(pthread_t)); 
        memcpy(threadData, &dataCopy, sizeof(tdata));
        int err = pthread_create(threadData->threadid, NULL, handleClient, (void *)threadData);
        if (err != 0)
