...
#include <fcntl.h>      /* for making servers sockets nonblocking*/
...
static int createServerSocket(unsigned short port)
{
    ...
    int val; 
    if ((val = fcntl(servSock, F_GETFD, 0)) < 0)
        die("fcntl f_get error"); 
    val |= O_NONBLOCK;
    if(fcntl(servSock, F_SETFD, val)< 0)
        die("fcntl g_set error");
