...
static char *getSocketName(int i) {
...
int main(int argc, char *argv[])
{
	...
	if (pid == 0) {
		...
		// create socket
		char *name = getSocketName(i);
		struct sockaddr_un un;
		int unixSocket, unLen;
		...
		if (bind(unixSocket, (struct sockaddr *)&un, unLen) < 0) 
	    	    die("bind failed");

		int clntSock = recvConnection(unixSocket);
...
loop_end:
	...
	if (clntSock < 0) {
		...
		while (sem_post(statSem) != 0) {}
		while (sem_close(statSem) != 0) {}
	    } else { die("accept failed"); }
	} else {
	    // create socket
	    char *name = getSocketName(rr);
	    struct sockaddr_un un;
	    int unixSocket, unLen;
	    if ((unixSocket = socket(AF_UNIX, SOCK_DGRAM, 0)) < 0)
	    	die("socket failed");	
	    ...
	    connect(unixSocket, (struct sockaddr *) &un, unLen);
	    sendConnection(clntSock, unixSocket);
	    close(clntSock);
	    if (++rr >= N_PROCESSES)
		rr = 0;
	}
